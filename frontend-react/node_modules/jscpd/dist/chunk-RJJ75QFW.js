...
function initCli(packageJson, argv) {
  ...
  ).option("-f, --format [string]", "format or formats separated by comma (Example php,javascript,python)").option("-p, --pattern [string]", "glob pattern to file search (Example **/*.txt)").option("-b, --blame", "blame authors of duplications (get information about authors from git)").option("-s, --silent", "do not write detection progress and result to a console").option("--store [string]", "use for define custom store (e.g. --store leveldb used for big codebase)").option("--store-path [string]", "directory to use for store cache (e.g. --store-path /tmp/jscpd-cache, useful when running multiple instances in parallel)").option("-a, --absolute", "use absolute path in reports").option("-n, --noSymlinks", "dont use symlinks for detection in files").option("--ignoreCase", "ignore case of symbols in code (experimental)").option("-g, --gitignore", "respect .gitignore files (default: enabled, use --no-gitignore to disable)").option("--no-gitignore", "do not respect .gitignore files").option("--colors", "force ANSI colors in console output even when stdout is not a TTY").option("--no-colors", "disable ANSI colors in console output (default: auto-detect TTY / NO_COLOR / FORCE_COLOR)").option("--formats-exts [string]", "list of formats with file extensions (javascript:es,es6;dart:dt)").option("--formats-names [string]", "list of formats with specific filenames (makefile:Makefile,GNUmakefile;docker:Dockerfile)").option("-d, --debug", "show debug information, not run detection process(options list and selected files)").option("-v, --verbose", "show full information during detection process").option("--list", "show list of total supported formats").option("--skipLocal", "skip duplicates in local folders, just detect cross folders duplications").option("--exitCode [number]", "exit code to use when code duplications are detected").option("--noTips", "do not print tips and promotional messages after detection").option("--skipComments", "ignore comments during detection (alias for --mode weak)");
...
function convertGitignorePatternToGlob(line) {
  if (line.startsWith("!")) {
...
function initIgnore(options) {
  ...
  if (options.gitignore && _fs.existsSync.call(void 0, _path.join.call(void 0, process.cwd(), ".gitignore"))) {
    const gitignorePath = _path.join.call(void 0, process.cwd(), ".gitignore");
    const content = _fs.readFileSync.call(void 0, gitignorePath, "utf8");
    const gitignorePatterns = content.split("\n").map((line) => line.trim()).filter((line) => line && !line.startsWith("#")).flatMap(convertGitignorePatternToGlob);
...
var resolveIgnorePattern = (configDir, pattern) => {
  ...
  const absolutePattern = _path.resolve.call(void 0, configDir, pattern);
  const cwd = process.cwd();
  const relativePath = _path.relative.call(void 0, cwd, absolutePattern);
...
var convertCliToOptions = (cli) => {
  const opts = cli.opts();
  const result = {
    minTokens: opts.minTokens ? parseInt(opts.minTokens) : void 0,
    minLines: opts.minLines ? parseInt(opts.minLines) : void 0,
    maxLines: opts.maxLines ? parseInt(opts.maxLines) : void 0,
    ...
    output: opts.output,
    format: opts.format,
    formatsExts: _finder.parseFormatsExtensions.call(void 0, opts.formatsExts),
    formatsNames: _finder.parseFormatsNames.call(void 0, opts.formatsNames),
    list: opts.list,
...
var readConfigJson = (config) => {
  ...
  if (configExists) {
    ...
    const configDir = _path.dirname.call(void 0, configFile);
    if (result.path) {
      result.path = result.path.map((path) => _path.resolve.call(void 0, configDir, path));
    }
    if (result.ignore) {
      result.ignore = result.ignore.map((pattern) => resolveIgnorePattern(configDir, pattern));
    }
    return result;
  }
  return {};
...
var readPackageJsonConfig = () => {
  ...
  if (_fs.existsSync.call(void 0, config)) {
    let json;
    try {
      json = _fsextra.readJSONSync.call(void 0, config);
    } catch (e) {
      console.warn(`Warning: Could not parse ${config}: ${e.message}`);
      return {};
    }
    const configDir = _path.dirname.call(void 0, config);
    if (json.jscpd && json.jscpd.path) {
      json.jscpd.path = json.jscpd.path.map((path) => _path.resolve.call(void 0, configDir, path));
    }
    if (json.jscpd && json.jscpd.ignore) {
      json.jscpd.ignore = json.jscpd.ignore.map((pattern) => resolveIgnorePattern(configDir, pattern));
    }
...
function prepareOptions(cli) {
  const storedConfig = readConfigJson(cli.opts().config);
...
function getStore(storeName, storePath) {
  if (storeName) {
...
var TIMER_LABEL = "time";
var detectClones = (opts, store = void 0, statisticProvider = void 0) => {
  const options = { ..._core.getDefaultOptions.call(void 0, ), ...opts };
  options.format = options.format || _tokenizer.getSupportedFormats.call(void 0, );
  options.mode = _core.getModeHandler.call(void 0, options.mode);
  if (typeof options.colors === "boolean") {
    _finder.configureColors.call(void 0, options);
  }
  ...
  const currentStore = store || getStore(options.store, options.storePath);
  const statistic = statisticProvider || new (0, _core.Statistic)();
  const tokenizer = new (0, _tokenizer.Tokenizer)();
  const detector = new (0, _finder.InFilesDetector)(
    tokenizer,
    currentStore,
    statistic,
    options
  ...
  _finder.registerSubscribers.call(void 0, options, detector);
  ...
  return detector.detect(files).then((clones) => {
    if (!options.silent) {
      ...
      if (!options.noTips) {
        console.log("");
        console.log(
          _safe.grey.call(void 0, "\u{1F4A1} Auto-refactor with AI: npx skills add kucherenko/jscpd")
        );
        console.log(
          _safe.grey.call(void 0, 
            "\u{1F3A9} New: Gangsta Agents \u2014 discipline your AI coding \u2192 gangsta.page"
          )
        );
        console.log(
          _safe.grey.call(void 0, "\u{1F496} Support jscpd project \u2192 https://opencollective.com/jscpd")
        );
      }
    }
    return clones;
...
var detectClonesAndStatistic = (opts, store = void 0) => {
  const statistic = new (0, _core.Statistic)();
  return detectClones(opts, store, statistic).then((clones) => ({
    clones,
...
exports.detectClones = detectClones; exports.detectClonesAndStatistic = detectClonesAndStatistic; exports.jscpd = jscpd;
//# sourceMappingURL=chunk-RJJ75QFW.js.map