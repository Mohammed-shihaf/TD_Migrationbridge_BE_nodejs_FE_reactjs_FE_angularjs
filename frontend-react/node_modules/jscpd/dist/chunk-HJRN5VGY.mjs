...
import {
  ...
  getModeHandler as getModeHandler2,
  Statistic
} from "@jscpd/core";
import { grey as grey3, italic } from "colors/safe";
import {
  getFilesToDetect,
  InFilesDetector as InFilesDetector3
} from "@jscpd/finder";
...
var readConfigJson = (config) => {
  ...
  if (configExists) {
    ...
    if (result.path) {
      result.path = result.path.map((path) => resolve(configDir, path));
    }
    if (result.ignore) {
...
var readPackageJsonConfig = () => {
  ...
  if (existsSync2(config)) {
    ...
    if (json.jscpd && json.jscpd.path) {
      json.jscpd.path = json.jscpd.path.map((path) => resolve(configDir, path));
    }
...
// src/init/colors.ts
import { shouldEnableColors, configureColors } from "@jscpd/finder";
...
import { registerSubscribers } from "@jscpd/finder";
...
var detectClones = (opts, store = void 0, statisticProvider = void 0) => {
  ...
  options.mode = getModeHandler2(options.mode);
  ...
  const detector = new InFilesDetector3(
    tokenizer,
    currentStore,
    statistic,
    options
  ...
  return detector.detect(files).then((clones) => {
    if (!options.silent) {
      ...
      if (!options.noTips) {
        console.log("");
        console.log(
          grey3("\u{1F4A1} Auto-refactor with AI: npx skills add kucherenko/jscpd")
        );
        console.log(
          grey3(
        ...
        console.log(
          grey3("\u{1F496} Support jscpd project \u2192 https://opencollective.com/jscpd")
        );
      }
    }
    return clones;
...
//# sourceMappingURL=chunk-HJRN5VGY.mjs.map