...
function registerSubscribers(options, detector) {
  ...
  if (!options.silent && !_optionalChain([options, 'access', _ => _.reporters, 'optionalAccess', _2 => _2.includes, 'call', _3 => _3("ai")])) {
...
async function jscpd(argv, exitCallback) {
  ...
  if (options.debug) {
    ...
    return detectClones(options, store).then((clones) => {
      if (clones.length > 0) {
        _optionalChain([exitCallback, 'optionalCall', _4 => _4(options.exitCode || 0)]);
      }
      return clones;
    }).finally(() => {
      store.close();
...
//# sourceMappingURL=chunk-LVL5XK2T.js.map