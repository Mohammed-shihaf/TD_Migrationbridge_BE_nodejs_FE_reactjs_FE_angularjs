...
var SkipLocalValidator = class _SkipLocalValidator {
  ...
  shouldSkipClone(clone, options) {
    const path2 = getOption("path", options);
    return path2.some(
...
var InFilesDetector = class {
  ...
  detect(fls) {
    ...
    const detect = (entry, clones = []) => {
      ...
      const format = entry.detectedFormat ?? getFormatByFile(path2, options.formatsExts, options.formatsNames);
...
function skipNotSupportedFormats(options) {
  return (entry) => {
    ...
    let format = getFormatByFile2(path2, options.formatsExts, options.formatsNames);
    if (format === void 0 && entry.stats?.mode !== void 0 && (entry.stats.mode & 73) !== 0) {
      if (!isSymlink(path2)) {
...
function skipBigFiles(options) {
  return (entry) => {
    ...
    if (options.debug && shouldSkip) {
      console.log(`File ${path2} skipped! Size more then limit (${bytes(stats.size)} > ${getOption2("maxSize", options)})`);
    }
    return !shouldSkip;
  };
}
  ...
  return (entry) => {
    ...
    if (lines < minLines || lines > maxLines) {
      if (options.debug || options.verbose) {
        console.log(grey(`File ${path2} skipped! Code lines=${lines} not in limits (${minLines}:${maxLines})`));
      }
      return false;
    }
    return true;
...
function addContentToEntry(entry) {
  const { path: path2 } = entry;
  const content = readFileSync(path2).toString();
...
function collectGitignorePatterns(dirs, globalExcludesFile) {
  ...
  const globalPath = globalExcludesFile === void 0 ? getGlobalGitIgnorePath() : globalExcludesFile ?? void 0;
...
function getPath(path2, options) {
  return options.absolute ? path2 : relative2(cwd(), path2);
}
function getPathConsoleString(path2, options) {
  return bold(green(getPath(path2, options)));
}
...
var JsonReporter = class {
  ...
  report(clones, statistic) {
    ...
    console.log(green2(`JSON report saved to ${join2(this.options.output, "jscpd-report.json")}`));
  }
  cloneFound(clone) {
...
var CSVReporter = class {
  ...
  report(clones, statistic) {
    ...
    console.log(green3(`CSV report saved to ${join3(this.options?.output, "jscpd-report.csv")}`));
  }
};
...
`;
    ...
    console.log(green4(`Markdown report saved to ${join4(this.options.output, "jscpd-report.md")}`));
  }
};
...
var XmlReporter = class {
  ...
  report(clones) {
    ...
    console.log(green5(`XML report saved to ${join5(this.options.output, "jscpd-report.xml")}`));
  }
};
...
// src/utils/colors.ts
import * as colors2 from "colors/safe";
...
function registerSubscribers(options, detector) {
  ...
  const reporterPrintsClones = options.reporters?.some(
