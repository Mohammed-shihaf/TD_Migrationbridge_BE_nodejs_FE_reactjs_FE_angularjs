...
var SkipLocalValidator = class _SkipLocalValidator {
  ...
  shouldSkipClone(clone, options) {
    const path2 = _core.getOption.call(void 0, "path", options);
    return path2.some(
  ...
  static isRelative(file, path2) {
    const rel = _path.relative.call(void 0, path2, file);
...
var InFilesDetector = (_class = class {
  ...
  detect(fls) {
    ...
    const detect = (entry, clones = []) => {
      const { path: path2, content } = entry;
      const format = _nullishCoalesce(entry.detectedFormat, () => ( _tokenizer.getFormatByFile.call(void 0, path2, options.formatsExts, options.formatsNames)));
      return format !== void 0 ? detector.detect(path2, content, format).then((clns) => {
        if (clns) {
          clones.push(...clns);
        }
...
var _safe = require('colors/safe'); var colors2 = _interopRequireWildcard(_safe);
var _fs = require('fs');
var _bytes = require('bytes'); var _bytes2 = _interopRequireDefault(_bytes);

var _child_process = require('child_process');
var SHEBANG_TO_FORMAT = {
  bash: "bash",
  sh: "bash",
  zsh: "bash",
  dash: "bash",
...
function readShebangFormat(filePath) {
  try {
    const buf = _fsextra.readFileSync.call(void 0, filePath, { encoding: null });
    const head = buf.slice(0, 128).toString("utf8");
    const firstLine = _nullishCoalesce(head.split("\n")[0], () => ( ""));
...
function isFile(path2) {
  try {
    const stat = _fs.lstatSync.call(void 0, path2);
    return stat.isFile();
  } catch (e) {
    return false;
  }
...
function skipNotSupportedFormats(options) {
  return (entry) => {
    ...
    let format = _tokenizer.getFormatByFile.call(void 0, path2, options.formatsExts, options.formatsNames);
    if (format === void 0 && _optionalChain([entry, 'access', _ => _.stats, 'optionalAccess', _2 => _2.mode]) !== void 0 && (entry.stats.mode & 73) !== 0) {
      if (!isSymlink(path2)) {
        format = readShebangFormat(path2);
    ...
    if ((options.debug || options.verbose) && !shouldNotSkip) {
      console.log(`File ${path2} skipped! Format "${format}" does not included to supported formats.`);
    }
...
function skipBigFiles(options) {
  return (entry) => {
    ...
    if (options.debug && shouldSkip) {
      console.log(`File ${path2} skipped! Size more then limit (${_bytes2.default.call(void 0, stats.size)} > ${_core.getOption.call(void 0, "maxSize", options)})`);
    }
    return !shouldSkip;
  };
}
  ...
  return (entry) => {
    ...
    if (lines < minLines || lines > maxLines) {
      if (options.debug || options.verbose) {
        console.log(_safe.grey.call(void 0, `File ${path2} skipped! Code lines=${lines} not in limits (${minLines}:${maxLines})`));
      }
      return false;
    }
    return true;
...
function addContentToEntry(entry) {
  const { path: path2 } = entry;
  const content = _fsextra.readFileSync.call(void 0, path2).toString();
  return { ...entry, content };
}
function gitignoreLineToGlobs(line, baseDir) {
...
function getGlobalGitIgnorePath() {
  try {
    const result = _child_process.execSync.call(void 0, "git config --global core.excludesFile", {
      encoding: "utf8",
...
function collectGitignorePatterns(dirs, globalExcludesFile) {
  const patterns = [];
  const visitedDirs = /* @__PURE__ */ new Set();
  const visitedRepos = /* @__PURE__ */ new Set();
  for (const dir of dirs) {
    ...
    while (true) {
      ...
      if (_fs.existsSync.call(void 0, path.join(current, ".git"))) {
        repoRoot = current;
        break;
      }
    ...
    for (const d of walkDirs) {
      ...
      if (!_fs.existsSync.call(void 0, gitignorePath)) continue;
      try {
        const content = _fsextra.readFileSync.call(void 0, gitignorePath, "utf8");
    ...
    if (repoRoot && !visitedRepos.has(repoRoot)) {
      ...
      if (_fs.existsSync.call(void 0, excludePath)) {
        try {
          const content = _fsextra.readFileSync.call(void 0, excludePath, "utf8");
          for (const line of content.split("\n")) {
  ...
  const globalPath = globalExcludesFile === void 0 ? getGlobalGitIgnorePath() : _nullishCoalesce(globalExcludesFile, () => ( void 0));
  if (globalPath && _fs.existsSync.call(void 0, globalPath)) {
    try {
      const content = _fsextra.readFileSync.call(void 0, globalPath, "utf8");
      for (const line of content.split("\n")) {
...
function getFilesToDetect(options) {
  ...
  if (options.noSymlinks) {
    patterns = patterns !== void 0 ? patterns.filter((path2) => !isSymlink(path2)) : [];
  }
  const scanDirs = (patterns || []).map((inputPath) => {
    try {
      return isFile(_fsextra.realpathSync.call(void 0, inputPath)) ? path.dirname(inputPath) : inputPath;
    } catch (e7) {
      return inputPath;
    }
  });
  patterns = patterns !== void 0 ? patterns.map((inputPath) => {
    const currentPath = _fsextra.realpathSync.call(void 0, inputPath);
    if (isFile(currentPath)) {
      return inputPath;
    }
    return inputPath.endsWith("/") ? `${inputPath}${pattern}` : `${inputPath}/${pattern}`;
  }) : [];
  const normalizedIgnore = (options.ignore || []).flatMap((ignorePattern) => {
    if (path.isAbsolute(ignorePattern) || ignorePattern.startsWith("**/")) {
      return [ignorePattern];
    }
    const variants = /* @__PURE__ */ new Set([ignorePattern]);
    for (const scanDir of [...scanDirs, "."]) {
      variants.add(path.join(scanDir, ignorePattern));
      variants.add(path.resolve(cwd2, scanDir, ignorePattern));
    }
    return [...variants];
  });
...
function getPath(path2, options) {
  return options.absolute ? path2 : _path.relative.call(void 0, _process.cwd.call(void 0, ), path2);
}
function getPathConsoleString(path2, options) {
  return _safe.bold.call(void 0, _safe.green.call(void 0, getPath(path2, options)));
}
  ...
  subscribe() {
    return {
      ...
      "CLONE_SKIPPED": (payload) => {
        ...
        console.log(
          _safe.grey.call(void 0, "Clone skipped: " + _optionalChain([validation, 'optionalAccess', _3 => _3.message, 'optionalAccess', _4 => _4.join, 'call', _5 => _5(" ")]))
        );
      },
      "START_DETECTION": (payload) => {
        ...
        console.log(
          _safe.grey.call(void 0, "Start detection for source id=" + _optionalChain([source, 'optionalAccess', _6 => _6.getId, 'call', _7 => _7()]) + " format=" + _optionalChain([source, 'optionalAccess', _8 => _8.getFormat, 'call', _9 => _9()]))
        );
      }
    };
  }
...
// src/utils/table-style.ts
var colors = __require("colors/safe");
var DEFAULT_HEAD = ["red"];
var DEFAULT_BORDER = ["grey"];
function getTableStyle() {
  return colors.enabled ? { head: [...DEFAULT_HEAD], border: [...DEFAULT_BORDER] } : { head: [], border: [] };
}
...
var ConsoleFullReporter = class {
  ...
  cloneFullFound(clone) {
    const table = new Table2({ ...TABLE_OPTIONS, style: getTableStyle() });
...
var JsonReporter = class {
  ...
  cloneFound(clone) {
    ...
    return {
      ...
      tokens: clone.duplicationA.end.position && clone.duplicationA.start.position !== void 0 ? clone.duplicationA.end.position - clone.duplicationA.start.position : 0,
      firstFile: {
...
var CSVReporter = class {
  ...
  report(clones, statistic) {
    ...
    console.log(_safe.green.call(void 0, `CSV report saved to ${_path.join.call(void 0, _optionalChain([this, 'access', _10 => _10.options, 'optionalAccess', _11 => _11.output]), "jscpd-report.csv")}`));
  }
};
    ...
    clones.forEach((clone) => {
      ...
      <duplication lines="${clone.duplicationA.end.line - clone.duplicationA.start.line}">
            <file path="${escapeXml(getPath(clone.duplicationA.sourceId, this.options))}" line="${clone.duplicationA.start.line}">
              <codefragment><![CDATA[${_optionalChain([clone, 'access', _12 => _12.duplicationA, 'access', _13 => _13.fragment, 'optionalAccess', _14 => _14.replace, 'call', _15 => _15(/]]>/i, "CDATA_END")])}]]></codefragment>
            </file>
            <file path="${escapeXml(getPath(clone.duplicationB.sourceId, this.options))}" line="${clone.duplicationB.start.line}">
              <codefragment><![CDATA[${_optionalChain([clone, 'access', _16 => _16.duplicationB, 'access', _17 => _17.fragment, 'optionalAccess', _18 => _18.replace, 'call', _19 => _19(/]]>/i, "CDATA_END")])}]]></codefragment>
            </file>
            <codefragment><![CDATA[${_optionalChain([clone, 'access', _20 => _20.duplicationA, 'access', _21 => _21.fragment, 'optionalAccess', _22 => _22.replace, 'call', _23 => _23(/]]>/i, "CDATA_END")])}]]></codefragment>
        </duplication>
      `;
    });
    xmlDoc += "</pmd-cpd>";
...
function compressCloneLine(pathA, pathB, rangeA, rangeB) {
...
var AiReporter = class {
  ...
  report(clones, statistic) {
    ...
    if (statistic) {
      console.log("---");
      console.log(
        `${clones.length} clones \xB7 ${statistic.total.percentage.toFixed(1)}% duplication`
      );
    }
  }
};
...
var parseFormatsNames = parseFormatsExtensions;

// src/utils/colors.ts

function shouldEnableColors(input) {
...
function configureColors(options = {}, env = process.env, isTTY = Boolean(process.stdout && process.stdout.isTTY)) {
...
// src/utils/register-subscribers.ts
var REPORTERS_PRINTING_CLONES = ["ai", "consoleFull"];
function registerSubscribers(options, detector) {
  if (options.verbose) {
    detector.registerSubscriber(new VerboseSubscriber(options));
  }
  const reporterPrintsClones = _optionalChain([options, 'access', _24 => _24.reporters, 'optionalAccess', _25 => _25.some, 'call', _26 => _26(
    (reporter) => REPORTERS_PRINTING_CLONES.includes(reporter)
  )]);
...
exports.AiReporter = AiReporter; exports.BlamerHook = BlamerHook; exports.CSVReporter = CSVReporter; exports.ConsoleFullReporter = ConsoleFullReporter; exports.ConsoleReporter = ConsoleReporter; exports.FragmentsHook = FragmentsHook; exports.InFilesDetector = InFilesDetector; exports.JsonReporter = JsonReporter; exports.MarkdownReporter = MarkdownReporter; exports.ProgressSubscriber = ProgressSubscriber; exports.SilentReporter = SilentReporter; exports.SkipLocalValidator = SkipLocalValidator; exports.ThresholdReporter = ThresholdReporter; exports.VerboseSubscriber = VerboseSubscriber; exports.XcodeReporter = XcodeReporter; exports.XmlReporter = XmlReporter; exports.collectGitignorePatterns = collectGitignorePatterns; exports.compressCloneLine = compressCloneLine; exports.configureColors = configureColors; exports.getFilesToDetect = getFilesToDetect; exports.parseFormatsExtensions = parseFormatsExtensions; exports.parseFormatsNames = parseFormatsNames; exports.registerSubscribers = registerSubscribers; exports.shouldEnableColors = shouldEnableColors;
